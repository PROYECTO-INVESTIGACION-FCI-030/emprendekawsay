...
export async function GET(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (signedError || !signed?.signedUrl) {
    return NextResponse.json({ ok: false, message: "No se pudo preparar la descarga" }, { status: 500 })
  }

  const response = await fetch(signed.signedUrl)
  if (!response.ok) {
    return NextResponse.json({ ok: false, message: "No se pudo descargar el archivo" }, { status: 500 })
  }
  ...
  return new NextResponse(buffer, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${documento.archivo_nombre.replace(/"/g, '\\"')}"`,
    },
  })
}
